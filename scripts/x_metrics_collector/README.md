...
> **現在はPhase 1の暫定評価フェーズ中のため、このコードは正式レーンとして稼働していない。** 日次運用は人間が見えている数値のスクリーンショットを渡す暫定レーンで回している。詳細は[ops/reports/provisional_evaluation_phase_2026-08-04.md](../../ops/reports/provisional_evaluation_phase_2026-08-04.md)参照。本コードは削除せず、X APIへの課金判断が下りた場合の将来の有料オプション候補として温存している。
