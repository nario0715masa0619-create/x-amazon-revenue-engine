...
1. 捏造:
   ...
   - 原文が明示していない結論の断定
   例外（この項の違反として扱ってはならないもの）: 生成文の末尾に付与された
   ハッシュタグ（#で始まる語句）、および読者の意見・経験・好みを尋ねる単純な
   問いかけの一文は、原文に存在しなくてもこの項の違反として報告しないこと。
   これらが「存在すること」自体を捏造として扱ってはならない。この問いかけを、
   上記の「使用場面」「比較軸」「体験の詳細」「結論の断定」のいずれかに
   分類し直して違反とすることも禁止する——問いかけの一文それ自体は、使用場面
   でも体験の詳細でも結論でもない。この例外は存在そのものにのみ適用される
   ——ハッシュタグや問いかけの文中に、原文に無いブランド名・型番・使用場面・
   比較軸・体験の詳細・数値・断定的結論が新たに含まれている場合は、その部分は
   例外の対象外としてこの項の違反であり続ける（例外はゼロ情報の付加要素のみを
   対象とし、新しい主張の追加を許可するものではない）。
...
6. 禁止文体:
   ...
   - CTA混入: 文中に「フォロー」「保存」「プロフィール」のいずれかを名指しして
     行動を求める定型的な誘導表現（例:「フォローしてね」「保存推奨」
     「プロフ見てね」「いいねしてね」）が含まれる場合にのみ、この項で違反とする。
     上記のような名指しの誘導が無い文は、内容が読者への問いかけであっても
     この項の対象外であり、違反として報告しないこと。「対話を誘発している」
     「エンゲージメントを狙っている」ことそのものは、この項の違反理由にしては
     ならない
7. コンプライアンス境界違反:
