...
### アカウント設計（2026-08-03確定、2026-08-04ターゲット改訂）

- 主対象: **40代男性**（2026-08-04改訂。旧: 40代男女。Google Sheets側の実データは別途更新済み）
- 重視する価値: 清潔感／上質感／実用性／無理のなさ／生活感を減らすこと（変更なし）
- 表現の文脈（例）: 仕事道具／通勤／持ち物の整い方／服より先に小物で生活感が出る、といった男性の日常シーンに寄せる
- 避ける表現: 若作り訴求／テンション高めの煽り／安さ一辺倒／若年層向け流行語の多用／**性別を決めつけた不自然な表現（極端に男性向けすぎる雑な固定観念も避ける）**

以降のresearcher/marketer/copywriter/compliance-reviewerの各判断は、このアカウント設計を前提とする。各担当のagent定義はこのセクションを参照する設計になっているため、個別のagentファイルを書き換える必要はない。

### 集客モードの評価思想（2026-08-04改訂）

集客モードの最優先目的は「信頼の入口を作ること」であり、そのためにはまず投稿を見つけてもらい、タイムライン上で止まってもらう必要がある。したがって、**集客モードの投稿評価は「破綻していないか」ではなく「競合比で強いか・弱いか・同等か」を中核判定とする。**

- 安全性・整合性チェック（コンプライアンス、アカウント設計との整合）は**必要条件**であり、それだけで`approved`にする理由にはしない
- 判定は絶対評価ではなく**相対評価**（競合比で強い／同等／弱い、の3段階）で行う
- 競合比較は**直接競合**（40代男性向けに服・持ち物・ガジェット・清潔感・身だしなみ・実用品を発信する近いアカウント）、**間接競合**（整理術・通勤・デスク環境・EDC・ミニマル持ち物・仕事道具など、同じ読者の注意を奪うアカウント）、**準ベンチマーク**（同業ではないがX上で短文フックが強く止める力のあるアカウント）の3層で行う
- 評価は**フック単体を先に評価**し、フックが競合比で「弱い」場合は原則としてその案を通さない。フックを通過した案のみ、本文全体・CTA・ターゲット適合・コンプライアンスまで進める
- 新しい評価5軸: **停止力**（一文目でタイムライン上で止まるか）／**自分事化**（40代男性が「それ自分のことだ」と感じるか）／**差別化**（よくある整理論・身だしなみ論・生活感論に埋もれないか）／**緊張感**（恥・損・見栄・失敗回避・他人の視線のいずれかが立っているか）／**遷移力**（プロフィールを見に行く理由が自然に生まれるか）

**集客モードの最低通過条件**: フック単体で競合比「同等以上」、投稿全体で競合比「同等以上」
**推奨条件**: 5軸のうち最低1つで「強い」。1つもなければ「整っているが弱い」可能性が高く、安易に`approved`にしない
**却下／差し戻し基準**（いずれかに該当する案は差し戻す）: フック単体で競合比「弱い」／40代男性の生活動線・仕事動線に着地せず抽象論のまま／競合と比べて差別化が見えない／安全だが平文で止まる理由がない／「他者の視線」「損失」「恥」「違和感」「具体利益」のどれにも触れていない／プロフィールへ飛ぶ理由が弱い

この判定思想は`market-grounded review layer`（[.claude/agents/trend-reality-reviewer.md](../../.claude/agents/trend-reality-reviewer.md)等）と`pre-post-self-check`（[.claude/skills/pre-post-self-check/SKILL.md](../../.claude/skills/pre-post-self-check/SKILL.md)）に反映されている。**コンプライアンス基準自体は弱めない**（affiliate-compliance-reviewerの最終権限・判定基準は変更なし）。
