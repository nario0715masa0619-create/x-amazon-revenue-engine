...
| growth-strategist | 前日実績、スプリント計画、current mode | 今日のテスト仮説／固定する要素・変える要素／今日の成功判定（**変える1変数が競合比のどの弱点を改善するためかを明示し、単なる比較可能性ではなく勝ち筋として意味のある差分かを判断する**）。**2026-08-07追加（Phase A試験運用）: 使用する価値カードID／固定する不変要素／試す可変要素（原則1つ）も明示する。詳細は[ops/reports/value_transfer_design_2026-08-07.md](../../../ops/reports/value_transfer_design_2026-08-07.md)参照** |
...
- [ ] （価値カードを使う日のみ）「使用する価値カードID」「固定する不変要素」「試す可変要素」がBriefに明記されているか（2026-08-07追加。Phase A試験運用）
