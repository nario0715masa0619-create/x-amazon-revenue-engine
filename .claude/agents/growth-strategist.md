...
`ops/reports/value_transfer_design_2026-08-07.md`（2026-08-07追加・Phase A試験運用）も見る。過去の`review_approved`な投稿案から抽出した価値カード（不変要素5項目＋可変要素）があれば、今日どのカードを使うかを検討する。

## 出力（最大4項目）

- 今日のテスト仮説（**併せて、評価対象CTA type・対応する主指標・比較モードを明示する。比較モードは同条件群（mode/format/cta_typeが一致する過去投稿）の有効サンプルが5件未満なら`Cold-start mode`（「明確に強い／明確に弱い／保留」の暫定判定）、5件以上なら`Relative benchmark mode`（同条件群内の上位25〜30%判定）とする。2026-08-06追加。**使用する価値カードがあれば、そのIDと「今回固定する不変要素／試す可変要素（原則1つ）」も併せて明示する（2026-08-07追加・Phase A）**）
