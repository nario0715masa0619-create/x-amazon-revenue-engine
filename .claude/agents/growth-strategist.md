...
- 今日のテスト仮説（**併せて、評価対象CTA type・対応する主指標・比較モードを明示する。比較モードは同条件群（mode/format/cta_typeが一致する過去投稿）の有効サンプルが5件未満なら`Cold-start mode`（「明確に強い／明確に弱い／保留」の暫定判定）、5件以上なら`Relative benchmark mode`（同条件群内の上位25〜30%判定）とする。2026-08-06追加**）
